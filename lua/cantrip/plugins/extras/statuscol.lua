      ...
      segments = {
        {
          ...
          condition = {
            function()
              return Cantrip.lsp.diagnostics_available() or " "
            end,
          },
        },
        {
            ...
            function(args)
              ...
              if normalized_mode == "v" then
                ...
                if vim.v.lnum >= s_row and vim.v.lnum <= e_row then
                  return Cantrip.hl_str("CursorLineNr", line)
                end
              end

              return vim.fn.line(".") == vim.v.lnum and Cantrip.hl_str("CursorLineNr", line)
                or Cantrip.hl_str("LineNr", line)
            end,
            " ",
          },
          condition = {
        ...
        },
        {
          ...
          condition = {
            function()
              local root = Cantrip.git.get_path_root(vim.api.nvim_buf_get_name(0))
              return Cantrip.git.get_git_remote_name(root) or " "
            end,
          },
        },
        {
